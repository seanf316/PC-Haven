...
class ProductViewsTestCase(TestCase):
    ...
    def test_update_profile_info(self):
        ...
        self.assertEqual(
            updated_profile.default_street_address1, "1 test house"
        )
        self.assertEqual(
            updated_profile.default_street_address2, "1 test street"
        )
    ...
    def test_add_to_wishlist_authenticated(self):
        ...
        self.client.login(username="sean", password="password")
        response = self.client.get(
            reverse("add_to_wishlist", args=[self.product.slug])
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response.url, reverse("product_detail", args=[self.product.slug])
        )
    ...
    def test_add_product_to_wishlist_that_contains_product_already(self):
        """
        Test adding a product to the wishlist when the
    ...
    def test_remove_from_wishlist_authenticated(self):
        """
        Test removing a product from the wishlist when
        ...
        response = self.client.get(
            reverse("remove_from_wishlist", args=[self.product.slug])
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response.url, reverse("product_detail", args=[self.product.slug])
    ...
    def test_remove_from_wishlist_when_product_not_in_wishlist(self):
        """
        Test removing a product from the wishlist when product
        is not in wishlist
        """
        self.client.login(username="sean", password="password")
        response = self.client.get(
        ...
        response = self.client.get(
            reverse("remove_from_wishlist", args=[self.product1.slug])
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response.url, reverse("product_detail", args=[self.product1.slug])
        )
    ...
    def test_clearing_wishlist(self):
        ...
        response = self.client.get(
            reverse("add_to_wishlist", args=[self.product1.slug])
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response.url, reverse("product_detail", args=[self.product1.slug])
